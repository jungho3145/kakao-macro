...
class CommentPlan:
    ...
    click_positions: 순차적으로 클릭할 좌표 목록.
        보통 [공지 목록 열기, 특정 공지 선택, 댓글 입력창] 3단계지만,
        사용자가 공지를 이미 연 상태라면 [댓글 입력창] 한 단계로 충분.
    comment_text: 댓글 본문 (UTF-8 한글 허용)
    step_delay_seconds: 클릭 사이 딜레이 (UI 렌더링 대기)
    submit_with_enter: True면 Ctrl 없이 Enter, False면 Ctrl+Enter (카톡 버전별 차이)
    """
...
def execute_plan(plan: CommentPlan, *, dry_run: bool = False) -> None:
    ...
    if plan.submit_with_enter:
        logger.info("  submit via Enter")
        if not dry_run:
            pyautogui.press("enter")
    else:
        logger.info("  submit via Ctrl+Enter")
        if not dry_run:
